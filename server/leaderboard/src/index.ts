...
try {
  ...
  const hive = new HiveAPI(config);
  const app = createRankingServer(hive, store, 30000, billing);
  const metrics = setInterval(() => console.log('[auth-metrics]', JSON.stringify(hive.authMetrics())), 60_000);
  metrics.unref();
