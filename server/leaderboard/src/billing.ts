...
export type Product = Record<string, unknown> & { id: string; android_product_id: string; type: string; consumable: boolean };
export interface BillingConfig { enabled: boolean; appId: string; zone: 'sandbox' | 'live'; key: string; allowReal: boolean }
...
interface Order { id: string; pid: string; app: string; sku: string; season: string }
interface Grant { tx: string; order_id: string; pid: string; app: string; sku: string; season: string; install: string; delivered: number; item: string }
export class Billing {
  ...
  private product(sku: unknown): Product {
    const p = this.products.find(p => p.android_product_id === sku);
  ...
  private check(data: Record<string, unknown>) {
    ...
    if (data.app_id !== this.config.appId) throw new APIError(400, '이 앱의 결제 설정을 확인해 주세요.');
  }
  ...
  async handle(action: string, pid: string, data: Record<string, unknown>): Promise<unknown> {
    ...
    if (action === 'entitlements') {
      const rows = this.db.prepare('SELECT item,season FROM iap_grants WHERE pid=? AND app=? AND delivered=1').all(pid, this.config.appId);
    ...
    if (action === 'order') {
      const item = this.product(data.sku), season = item.type === 'season_pass' ? seasonNow() : '';
      if (!item.consumable || season) {
        const owned = this.db.prepare('SELECT tx FROM iap_grants WHERE pid=? AND app=? AND sku=? AND season=?').get(pid, this.config.appId, item.android_product_id, season);
      ...
      let order = this.db.prepare('SELECT o.* FROM iap_orders o LEFT JOIN iap_grants g ON g.order_id=o.id WHERE o.pid=? AND o.app=? AND o.sku=? AND o.season=? AND g.tx IS NULL ORDER BY o.created DESC LIMIT 1').get(pid, this.config.appId, item.android_product_id, season) as unknown as Order | undefined;
      if (!order) {
        order = { id: randomUUID(), pid, app: this.config.appId, sku: item.android_product_id, season };
        this.db.prepare('INSERT INTO iap_orders VALUES(?,?,?,?,?,?)').run(order.id, pid, order.app, order.sku, season, Date.now());
      }
    ...
    if (action === 'ack') {
      ...
      if (!grant || grant.pid !== pid || grant.app !== this.config.appId || grant.install !== install) throw new APIError(403, '구매 기기와 계정을 확인해 주세요.');
    ...
    if (verified.result !== 0 || Number(verified.hiveiap_market_id) !== 2 || verified.hiveiap_purchase_cancel_state !== 0 || verified.hiveiap_account_uuid_compare !== 1)
      throw new APIError(422, '유효한 Google Play 구매를 확인하지 못했습니다.');
    ...
    if (purchase.packageName !== this.config.appId) throw new APIError(422, '구매 앱이 일치하지 않습니다.');
    ...
    if (!order || order.pid !== pid || order.app !== this.config.appId || order.sku !== verified.hiveiap_market_pid)
      throw new APIError(403, '구매 계정과 상품이 일치하지 않습니다.');
    const item = this.product(order.sku), tx = verified.hiveiap_transaction_id;
    ...
    try {
      ...
      if (grant && (grant.tx !== tx || grant.pid !== pid || grant.app !== order.app || grant.sku !== order.sku)) throw new APIError(409, '중복 주문입니다. 고객 지원에 문의해 주세요.');
      if (!grant) {
        if ((!item.consumable || order.season) && this.db.prepare('SELECT tx FROM iap_grants WHERE pid=? AND app=? AND sku=? AND season=?').get(pid, order.app, order.sku, order.season))
          throw new APIError(409, '이미 지급된 상품입니다. 고객 지원에 문의해 주세요.');
        this.db.prepare('INSERT INTO iap_grants VALUES(?,?,?,?,?,?,?,0,?)').run(tx, order.id, pid, order.app, order.sku, order.season, install, JSON.stringify(item));
