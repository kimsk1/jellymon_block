...
func run() -> void:
	...
	for id in ["a", "b", "d"]:
		...
		for icon in nav_icons:
			check(icon.botanical == (id == "b"), "Wrong navigation icon family")
		...
		if id == "b":
			...
			check(purchase.get_theme_stylebox("normal").bg_color == Color("#43bfb0"),"Popup primary color mismatch")
			purchase.free()
			await snap("B_shop")
			home._close_shop_popup()
			home._show_home_menu()
			await snap("B_settings")
	...
	print("[home theme UI] errors=", errors.size())
...
func snap(filename: String) -> void:
	...
	var path := "res://output/theme-b/" + filename + ".png"
