...
func _ready() -> void:
	...
	body.add_child(nav)
	var back := _button(L10n.text("‹ 지도"), 95)
	back.pressed.connect(func(): main.show_map(true))
	nav.add_child(back)
	var title := _label(L10n.text("모험 랭킹"), 32)
	title.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	nav.add_child(title)
	_refresh = _button(L10n.text("새로고침"), 115)
	...
	var rule := _label(L10n.text("별 3개로 깬 최고 레벨 순\n같은 레벨이면 먼저 달성한 유저가 앞서요"), 20)
	...
	for item in [[L10n.text("순위"), 56], [L10n.text("유저"), 0], [L10n.text("레벨"), 95], [L10n.text("최초 달성"), 180]]:
	...
	var note := _label(L10n.text("별 1·2개 기록은 제외 · 달성 시각은 한국시간 (UTC+9)\n기존 기록의 달성 시각이 없으면 별 3개로 다시 클리어해 주세요."), 16)
...
func _render() -> void:
	_status.text = L10n.text(main.ranking.status.replace("기기 시간대 기준", "한국시간 기준"))
	_refresh.disabled = main.ranking.loading
	var record: Dictionary = main.save.three_star_ranking_record()
	_mine.text = L10n.text("내 별 3개 기록: LEVEL %d") % int(record.level) if not record.is_empty() else L10n.text("아직 등록할 별 3개 달성 기록이 없어요")
	...
	for row in main.ranking.entries:
		...
		var items := [[str(index), 56, 26], [String(row.get("nickname", L10n.text("구조 대원"))) + (L10n.text(" · 나") if mine else ""), 0, 22], [str(int(row.level)) + "\n★★★", 83, 21], [date, 166, 17]]
		for item in items:
