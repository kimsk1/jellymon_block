...
func _ready() -> void:
	...
	for option in ["클라우드 사용", "기기 데이터 사용", "나중에"]:
		...
		button.pressed.connect(func():
			if option != "나중에": selected.emit(option == "클라우드 사용")
			queue_free()
		)
