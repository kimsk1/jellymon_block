...
| supporter.png | 구조대 후원 팩 전용 가구(임시 아트, `tools/generate_supporter_art.gd`로 생성) | 2 |
...
검증 씬: `tools/verify_furniture_art.tscn`. 전체 ID와 투명도, 52종 × 4방향의 점유 영역과 종횡비, 실제 홈 화면 및 꾸미기 회전·삭제를 확인한다. `--render-furniture`를 전달하면 `output/furniture-refresh/`에 A/B/D 배치 화면, 꾸미기 목록, 전체 가구 도감을 저장한다. 플레이어 저장 파일을 읽거나 수정하지 않는 테스트용 저장 객체를 사용한다.

### 검증 결과 (2026-09-08)

- 가구 52종 전체 이미지 및 실제 알파 채널 확인.
- 52종 × 4방향, 총 208개 점유 영역·종횡비 검사 통과.
